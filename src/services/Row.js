    ...
    while (!this.isFull()) {
      ...
      this.intervals.splice(
        ...
        ...[
          ...
          new Interval(
            interval.from + shift,
            interval.from + shift + size - 1,
            new Block(Row._pick(COLORS), size, interval.from + shift, this.y)
          ),
          ...(shift + size < interval.size()
            ? [new Interval(interval.from + shift + size, interval.to)]
            : [])
        ]
      )
    }
